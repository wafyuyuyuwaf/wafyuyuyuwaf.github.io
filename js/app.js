...
if(!x)return'[video]';if(x.url)return'<div style="padding-bottom:56.25%;position:relative"><iframe src="'+x.url+'" style="position:absolute;top:0;left:0;width:100%;height:100%;border:0;border-radius:8px;"></iframe></div>';
...
function hp(pw){var h=0;for(var i=0;i<pw.length;i++){h=((h<<5)-h)+pw.charCodeAt(i);h|=0;}return'h'+Math.abs(h).toString(36);}
function li(){return sessionStorage.getItem(SS)==='1';}
function il(){return location.protocol==='file:'||location.hostname==='localhost'||location.hostname==='127.0.0.1';}
function dd(){return{posts:[],settings:{blogTitle:'日志',blogSubtitle:'',adminPasswordHash:hp('1234566678'),categories:['见闻','随笔','攻略','创作','考据']}};}
var dt=ld();
